...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { email, password, name, guest_user_id, role } = req.body;
    ...
    // Transfer guest products to permanent user if guest_user_id provided (Madde 2, 3)
    if (guest_user_id && guest_user_id !== uuid && role === "GUEST") {
      try {
        console.log(
